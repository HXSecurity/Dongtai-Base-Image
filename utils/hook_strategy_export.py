...
select id from iast_hook_type iht where  language_id  = 1 AND enable in (0,1) and `type` in (1,2,3,4) )
and hookstrategy_id  in (select id from iast_hook_strategy ihs where created_by = 1 and enable in (0,1)));
'''
...
`type`=%s AND `name`= %s
AND  value = %s AND create_time= %s AND
update_time = %s  AND `created_by`= %s
...
`value` = %s AND `source` = %s AND `target` = %s AND `inherit` = %s AND `track` = %s AND `create_time` = %s AND `update_time` = %s AND `created_by` = %s AND `enable` = %s LIMIT 1);
...
finsql = hook_strategy_sql.format(
    **{
        i: '%s'
        for i in [
            'value', 'source', 'target', 'inherit', 'track', 'create_time',
            'update_time', 'created_by', 'enable'
        ]
    })
